...
public class ReservationController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ReservationDTO> getReservation(@PathVariable int userId, @PathVariable int courtId) {
        try {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ReservationDTO> updateReservation(@PathVariable int userId, @PathVariable int courtId,
                                                            @RequestBody ReservationDTO reservation) {
        try {
            return ResponseEntity.ok(reservationService.updateReservation(userId, courtId, reservation));
        } catch (Exception exception) {
            return ResponseEntity.notFound().build();
        }
    }
